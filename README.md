...
- **已有歌自动下一首**：录到府库里已有的歌（跨整个曲库精确比对，覆盖历史成品与你导入的专辑），自动让音源切到下一首，不在重复曲目上耗时间；可在设置里关掉，关掉则只标「跳过」继续往下录
- **成品即发布级**：LAME 320k 编码，内录音量自动归一化（不随系统音量忽大忽小），ID3 封面标签内嵌（系统不上报封面时自动从音源本地补齐，如酷我），成品旁挂同步歌词
...
**Q：录出来的 MP3 音质如何？**
LAME 320kbps CBR，与源音频同为 44.1kHz 采样率，内嵌 ID3v2.3 封面与标签，旁挂 `.lrc` 同步歌词。录音段会自动做响度归一化，不会因为录制时系统音量小就得到一批声音很小的成品。

**Q：录制时有歌被"跳过"是怎么回事？**
说明府库里已经有这首歌了（按「艺术家 - 歌名」跨整个曲库精确比对）。默认会自动让当前音源切到下一首（可在 设置 → 录制 →「已有歌自动下一首」关闭）；关掉后只标「跳过」并继续往下录。
